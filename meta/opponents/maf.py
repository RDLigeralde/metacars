...
class IAF(nn.Module):
    ...
    # alg is equiv of log prob for maf, arguments is same as inverse() in MAF
    def sample(self, u, y=None):
        return self.forward(u, y)

    # alg is equiv of sample for maf, arguments is the same as log_prob in MAF
    def log_prob(self, x, y=None):
        u, sum_log_abs_det_jacobians = self.net.inverse(x, y)
        return torch.sum(self.base_dist.log_prob(u) + sum_log_abs_det_jacobians, dim=1)
